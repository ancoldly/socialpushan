<div class="grid items-center bg-white rounded-[10px] p-[20px] gap-[20px] relative">
    <div class="flex items-center justify-between">
        <div class="flex gap-[20px] items-center">
            ...
            <div class="gird items-center">
                <form action="{{ route('show-profileDetails') }}" method="GET">
                    ...
                    <button type="submit" class="font-semibold flex items-center gap-[5px]">{{ $post->user->name }}<i
        ...
        <div class="relative">
            ...
            <div class="w-max h-max bg-gray-200 p-[20px] absolute top-[100%] right-[40px] rounded-[10px] rounded-tr-[0px] hidden gap-[20px] text-left justify-start cogPost"
                ...
                <form method="POST" action="{{ route('delete-post') }}" data-post-id="{{ $post->id }}"
                    ...
                    <form id="editPostShare-form" action="{{ route('edit-post-share') }}"
                        ...
                        <div class="flex items-center gap-[10px] cursor-pointer justify-end">
                            ...
                            <button id=""
                                ...
                                type="submit" name="submit-edit-post"><i class='bx bx-edit text-[24px]'></i>Edit
                                Post</button>
                        </div>
                    </form>
                @else
                    <form id="editPost-form" action="{{ route('edit-post') }}"
                        ...
                        <div class="relative">
                            <input type="hidden" name="current-image" id="current-image-{{ $post->id }}"
                                value="{{ $post->image_url }}">
                            <img id="imageEditPost-preview-{{ $post->id }}" src="{{ $post->image_url }}"
                                alt="" class="rounded-[10px] imageEditPost-preview"
                                data-post-id="{{ $post->id }}">
                        ...
                        <div class="flex items-center justify-between">
                            <div class="flex items-center gap-[10px] cursor-pointer">
                                <input type="file" id="imageEdit-post-{{ $post->id }}" name="imageEdit-post"
                                    value="{{ $post->image_url }}" accept="image/*" class="hidden"
            ...
            <div class="grid gap-[20px] border-[2px] p-[10px] rounded-[10px]">
                ...
                <div class="flex gap-[20px] items-center">
                    ...
                    <div class="gird items-center">
                        <form action="{{ route('show-profileDetails') }}" method="GET">
                            @csrf
                            <input type="hidden" name="userId" value="{{ $post->originalPost->user->id }}">
                            <button type="submit"
    ...
    @else
        @if ($post->content !== null || $post->image_url !== null)
            <div class="grid gap-[20px] border-[2px] p-[10px] rounded-[10px]">
                ...
                <div>
                    <img src="{{ $post->image_url }}" alt="" class="w-auto h-auto rounded-[10px]">
                </div>
            </div>
        @endif
    @endif
    ...
    <div class="flex items-center justify-between border-b-[2px] pb-[10px]">
        ...
        @if (Auth::check() && Auth::id() !== $post->user_id)
            @if ($post->parent_post_id !== null)
                @if ($post->originalPost)
                    <form class="sharePost-form flex items-center gap-[20px]" action="{{ route('create-share') }}"
                        data-post-id="{{ $post->originalPost->id }}" method="POST">
                        @csrf
                        <input type="hidden" name="post_Id" value="{{ $post->originalPost->id }}">
                        <button class="w-[40px] h-[40px] bg-gray-500 rounded-full text-white text-[24px] bx bx-share"
                            type="submit"></button>
                    </form>
                @endif
            @else
    ...
    <form class="createComment-form flex items-center" method="POST" action="{{ route('create-comment') }}"
        data-post-id="{{ $post->id }}">
        @csrf
        <div class="w-[10%]">
    ...
    <div class="overflow-y-scroll scroll-container max-h-[300px] grid gap-[10px] items-start">
        @foreach ($post->comments as $comment)
            <div class="flex items-center">
                <div class="w-[10%]">
                    <img src="{{ $comment->user->avatar ? $comment->user->avatar : $avatar_temp }}" alt=""
                ...
                <div class="bg-gray-200 p-[10px] rounded-[10px] w-[90%]">
                    <form id="editComment-form" action="{{ route('edit-comment') }}"
                        ...
                        class="hidden items-center editComment-form rounded-[10px] my-[10px] w-full" method="POST">
                        @csrf
                        <div class="w-full grid gap-[10px] text-left items-start">
                            ...
                            <button class="text-blue-500 text-[14px] cancel-editComment w-max" id="cancel-editComment"
                    ...
                    <div class="flex items-center justify-between relative">
                        ...
                        <div class="cogComments hidden bg-blue-50 gap-[10px] p-[10px] text-left rounded-[10px] absolute top-[0] right-[30px]"
                            ...
                            <button data-edit-id="{{ $post->id }}" data-id-comments="{{ $comment->id }}"
                                ...
                                Edit
                                Comments</button>
                            <form action="{{ route('delete-comment') }}" method="POST" class="deleteComment-form"
                                data-id-comments="{{ $comment->id }}">
                                @csrf
                                @method('DELETE')
                                <input type="hidden" name="commentsId" value="{{ $comment->id }}">
                                <button type="submit" class="flex items-center gap-[10px] font-semibold"><i
